...
use tool_forced_settings\local\loaders\json_loader;
 ...
 * @covers     \tool_forced_settings\local\loaders\json_loader
 */
final class json_loader_test extends \basic_testcase {
    ...
    /**
     * Test loading empty JSON object returns array with moodle and tool_forced_settings sections.
     */
    public function test_load_empty_json_object(): void {
        $result = $this->loader->load($this->get_fixture('empty.json'));

        $this->assertIsArray($result);
        // Class json_loader always adds 'moodle' and 'tool_forced_settings' sections.
        $this->assertArrayHasKey('moodle', $result);
        $this->assertArrayHasKey('tool_forced_settings', $result);
    }
    ...
    /**
     * Test that loader adds tool_forced_settings section with configfile path.
     */
    public function test_loader_adds_configfile_path(): void {
        $result = $this->loader->load($this->get_fixture('valid_config.json'));

        $this->assertArrayHasKey('tool_forced_settings', $result);
        $this->assertArrayHasKey('configfile', $result['tool_forced_settings']);
        $this->assertStringContainsString('valid_config.json', $result['tool_forced_settings']['configfile']);
    }
